...
def sanitize(config, string):
    if not config["nice-format"]:
        illegal_symbols = ["/"]
        return "".join([symbol for symbol in string if symbol not in illegal_symbols])
    alphanumeric = "".join(
        [
            character
...
def search(client, config, media_type, query):
    ...
    media_handlers = {
        models.Song: song_handler,
        models.Album: album_handler,
        models.Artist: artist_handler,
    }
    answer = questionary.select(
        "Select one:", choices=media_handlers[media_type](results)
    ).ask()
    if not answer:
...
def get_song_path(
    client, config, song, siblings=None, include_artist=False, include_album=False
):
    ...
    if include_artist or config["full-structure"]:
        artist_path = sanitize(config, song.album.get_primary_artist_name(config))
...
def download_song(
    ...
    with open(temporary_path, mode="wb") as pointer:
        ...
        if not config["quiet"]:
            ...
            iterator = tqdm(
                iterator, total=total, unit="KiB", unit_scale=True, dynamic_ncols=True
            )
...
def download_album(client, config, album, include_artist=False, indent=0):
    ...
    for index, song in enumerate(songs, 1):
        log(
            config,
            f'Downloading "{song.name}" ({index} of {len(songs)} song(s))...',
            indent=indent,
        )
        download_song(
            client,
...
def load_config(args):
    ...
    with open(config_file) as config_pointer:
        ...
        config = {
            key: resolve(args.get(key), file.get(key), default)
